...
const formFields: FormField[] = [
    ...
    { fieldName: 'email', label: 'Почта', required: true, type: 'text', name: 'email' },
    { fieldName: 'codeSm', label: 'Код SM', required: false, type: 'text' },
    { fieldName: 'codeSymphony', label: 'Код Symphony', required: false, type: 'text' },
...
function RegistationUser() {
    ...
    const { jobTitles, data, isPending: isPendingJT, errorMessage: errorMessageJT } = useJobTitles();
    ...
    useEffect(() => {
        if (data) {
            ...
            setJobTitleOptions(data.data.map((jT, index) => ({
                id: String(index + 1),
                name: jT,
            })));
            done();
    ...
    const renderFormField = (field: typeof formFields[0]) => {
        const commonProps = {
            ...
            ...(field.required && {
                blured: () => validateField(field.fieldName, registerUserModel[field.fieldName])
            })
        }
        return (
            <FormRowLayout
